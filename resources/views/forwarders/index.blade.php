...
@section('toolbar')
	<a href="{{route('forwarders.create')}}"><i class="fa fa-plus"></i></a> 
		    ...
		    <tbody>
		        ...
		        <tr style='height:50px'>
		            ...
		            <td class="txt-s text-center">
		               <a href="{{ route('forwarders.edit', $forwarder->id)}}"><i class="fa fa-pencil-alt text-success pt-2"></i></a>
		            </td>
		            <td class="text-center txt-s">
		                <form class='delete' action="{{ route('forwarders.destroy', $forwarder->id)}}" method="post" onsubmit="return confirm('Are you sure?')">
		                  @csrf
		                  @method('DELETE')
